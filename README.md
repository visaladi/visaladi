
<h3 align="center">"Passionate Multidisciplinary Professional:Software Architect | Frontend & Backend Developer | AI Engineer | ML Engineer | Electronic Enthusiast | App Developer"</h3>
<img align="center" alt="Coding" width="1000" height="300" src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExOTRlNGh5bHRmNjJ0amFnejZrcHQ5bzc3cTl2M3R4OWxrYTlrOXNmayZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/bGgsc5mWoryfgKBx1u/giphy.gif"> 
# 🚀 Hi, I'm Visal Sandeep Adikari

**Engineering Innovator | AI & Computer Vision Researcher | Blockchain Developer | Software Architect | Entrepreneur**

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=visaladi&label=Profile+Views&color=1c8adb&style=for-the-badge" alt="Profile Views" />
  <img src="https://img.shields.io/badge/Location-Sri%20Lanka-blue?style=for-the-badge" alt="Location" />
  <img src="https://img.shields.io/badge/Role-Engineering%20Leader-success?style=for-the-badge" alt="Role" />
</p>

---

## 🔥 About Me
I’m a multidisciplinary engineer passionate about building impactful technology that merges:

- 🤖 Artificial Intelligence, Deep Learning & Computer Vision  
- 🧠 Algorithm Engineering & Research  
- 🌐 Full-Stack Software Architecture  
- 🔐 Blockchain & Decentralized Systems  
- ⚙️ Embedded Systems, Electronics & Robotics  
- 🧑‍💼 Entrepreneurship, Product Thinking & Tech Leadership

I focus on turning research into production-ready systems that solve real problems and push innovation boundaries.

---

## 🎯 Mission
To build advanced engineering solutions that:
- Solve meaningful problems
- Push innovation boundaries
- Empower people & industries
- Represent Sri Lankan engineering excellence globally

---

## 📌 Currently
- R&D in AI, ML & Computer Vision  
- Blockchain solutions & cryptographic systems  
- Engineering high-impact products and research collaborations  
- Mentoring university & industry-level research work

---

## 🚀 Highlights
- Architected end-to-end systems combining cloud, backend, and embedded components  
- Research and applied projects in Computer Vision and Deep Learning  
- Designed blockchain based solutions and security tooling  
- Mentored students and led cross-functional engineering teams

---

## 🔭 Featured Projects
> Replace the links with your actual repository URLs or pin projects on your GitHub profile.

- DeepVision — High-performance computer vision and detection pipelines.  
  https://github.com/visaladi/DEEPVISION
- EdgeSense — Embedded vision and on-device ML for resource-constrained hardware.  
  https://github.com/visaladi/EDGESENSE
- ChainGuard — Blockchain cryptography and audit tooling.  
  https://github.com/visaladi/CHAINGUARD

---

## 🧠 Research & Publications
I actively pursue research in computer vision, model efficiency, and applied ML systems.  
If you are interested in papers, preprints, or collaboration, please reach out by email.

---

## 🛠️ Tech Arsenal

**Programming & Backend:** Java • Python • C / C++ • JavaScript • TypeScript • Node.js • Spring Boot  
**AI / ML / CV:** Deep Learning • PyTorch • TensorFlow • OpenCV • Model Optimization • Edge ML  
**Frontend:** React • Next.js • Tailwind / Bootstrap  
**Databases:** MongoDB • MySQL • PostgreSQL  
**DevOps / Cloud:** Docker • Azure • CI/CD  
**Embedded / Robotics:** Arduino • Firmware • Hardware Systems

---

## 📊 GitHub Profile Analytics

<p align="center">
  <img src="https://streak-stats.demolab.com?user=visaladi&theme=tokyonight&hide_border=true" alt="streak" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=visaladi&show_icons=true&theme=tokyonight&count_private=true&include_all_commits=true" alt="GitHub stats" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=visaladi&layout=compact&theme=tokyonight" alt="Top languages" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=visaladi&theme=tokyo-night" alt="Activity graph" />
</p>

---

## 🤝 Collaboration
I'm open to:
- Research collaborations in CV & ML
- Engineering leadership and architecture roles
- Blockchain and cryptography projects
- Mentorship, internships, and student collaborations

If you have a project or idea, let's talk — I enjoy collaborating across research and product domains.

---

## 📬 Reach Me
- Email: **amdbadikari@gmail.com**  
- LinkedIn: https://www.linkedin.com/in/visal-adikari-47930a234  
- X / Twitter: https://x.com/adikari_visal

---

## 📄 Resume & Hiring
Resume / CV available upon request — email me or add a public link here.

---

Thank you for visiting — always learning. always building. always evolving. 🌟
<p><img align="left" src="https://github-readme-stats.vercel.app/api/top-langs?username=visaladi&show_icons=true&locale=en&layout=compact" alt="visaladi" /></p>

<p>&nbsp;<img align="center" src="https://github-readme-stats.vercel.app/api?username=visaladi&show_icons=true&locale=en" alt="visaladi" /></p>

<p><img align="center" src="https://github-readme-streak-stats.herokuapp.com/?user=visaladi&" alt="visaladi" /></p>

# 📊 GitHub Stats:

|     ![](https://github-readme-stats.vercel.app/api?username=visaladi&theme=dark&hide_border=false&include_all_commits=true&count_private=true)        |       ![](https://github-readme-streak-stats.herokuapp.com/?user=visaladi&theme=dark&hide_border=false)                                                                                                 |
| --------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=visaladi&theme=dark) | ![](https://github-contributor-stats.vercel.app/api?username=visaladi&limit=5&theme=dark&combine_all_yearly_contributions=true) |

<img src="https://github-readme-activity-graph.vercel.app/graph?username=visaladi&radius=16&theme=github-compact&area=true&order=5" height="250" alt="activity-graph graph" />
